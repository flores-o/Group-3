...
from BinaryTreeStoringParents import BinaryTreeStoringParents
from NodeWithParent import NodeWithParent

class testBinaryTree(unittest.TestCase):

    def createExampleTree(self):
        binaryTree = BinaryTreeStoringParents(NodeWithParent(7))

        binaryTree.insert(NodeWithParent(3), binaryTree.root, True)
        binaryTree.insert(NodeWithParent(4), binaryTree.root, False)
        binaryTree.insert(NodeWithParent(2), binaryTree.root.left, True)
        binaryTree.insert(NodeWithParent(5), binaryTree.root.left, False)
        binaryTree.insert(NodeWithParent(8), binaryTree.root.right, False)
        binaryTree.insert(NodeWithParent(1), binaryTree.root.left.left, True)
        binaryTree.insert(NodeWithParent(6), binaryTree.root.left.left, False)
    ...
    def test_insertLeft(self):
        node1 = NodeWithParent(1)
        binaryTree = BinaryTreeStoringParents(node1)

        # Node is inserted successfully
        node2 = NodeWithParent(2)
